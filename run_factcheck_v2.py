"""
KI-Produkt-Faktencheck · Automatisches Fact-Checking V2

HYBRID-RECHERCHE:
- Perplexity: Schnelle Web-Suche + aktuelle Quellen
- Google Scholar: Wissenschaftliche Papers für spezifische Claims

Reduziert "NICHT_PRÜFBAR"-Rate durch Fallback-Strategie.
"""
...
try:
    ...
    print("Fehler: pip install anthropic pandas scholarly")
    sys.exit(1)
...
- Bevorzuge wissenschaftliche Quellen (Google Scholar) über Web-Quellen
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    return None  # Signal für Fallback
    except Exception as e:
        return None  # Signal für Fallback


async def search_google_scholar(claim: str, max_results: int = 5) -> str:
    """Sucht wissenschaftliche Papers via Google Scholar als Fallback."""
    try:
        # Scholar-Suche im separaten Thread (ist blocking)
        loop = asyncio.get_event_loop()

        def _search():
            # Keywords extrahieren (sehr simpel)
            keywords = claim[:100]  # Erste 100 Zeichen als Query
            ...
            return "GOOGLE SCHOLAR ERGEBNISSE:\n\n" + "\n".join(results)
...
async def hybrid_search(claim: str, perplexity_key: str) -> Tuple[str, str]:
    ...
    Returns:
        (sources_text, source_type)  # source_type: "perplexity", "scholar", "both", "none"
    """

    # Versuche Perplexity zuerst (schneller)
    perplexity_result = await search_web_perplexity(claim, perplexity_key)
    ...
    scholar_result = await search_google_scholar(claim, max_results=3)

    if scholar_result:
        return scholar_result, "scholar"

    # Keine Quellen gefunden
    return "Keine Quellen verfügbar (weder Perplexity noch Google Scholar).", "none"
...
async def factcheck_claim_async(
    ...
    """Fact-checkt einen einzelnen Claim mit Hybrid-Recherche."""
    ...
    async with semaphore:
        ...
        sources, source_type = await hybrid_search(claim_text, perplexity_key)

        # Quellen-Qualität basierend auf Typ
        quellen_qualitaet_map = {
            "perplexity": "gut",
            "scholar": "gut",
            "both": "sehr_gut",
            "none": "schwach"
        }
        expected_quality = quellen_qualitaet_map[source_type]
...
async def main_async(args):
    ...
    if not perplexity_key:
        print("⚠️  Warnung: PERPLEXITY_API_KEY nicht gesetzt. Nur Google Scholar wird verwendet.")
        perplexity_key = "mock"
...
def main():
    ap = argparse.ArgumentParser(description="Automatisches Fact-Checking V2 (Hybrid)")
    ap.add_argument("--mode", choices=["sample", "priority", "all"], required=True)
    ap.add_argument("--input", default="claims_canonical.csv")
    ap.add_argument("--output", default="claims_factchecked_v2.csv")
